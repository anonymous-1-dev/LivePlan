...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        ...
        # If the model is OpenRouter, sanitize messages
        # For Anthropic models, preserve cache_control for prompt caching
        from sweagent._openrouter_shim import _is_openrouter_target, sanitize_messages_for_openrouter
        if _is_openrouter_target(self.config.name, self.config.api_base):
            completion_kwargs["max_tokens"] = self.model_max_output_tokens
            messages = sanitize_messages_for_openrouter(messages, model_name=self.config.name)
        try:
...
class OpenAIModel(AbstractModel):
    ...
    def query(self, history: History, n: int = 1, temperature: float | None = None) -> list[dict] | dict:
        ...
        try:
            ...
            raise ModelConfigurationError(f"OpenAI call failed: {e}")

        outputs = []
        for choice in response.choices:
            # Ensure message content is always a string (not None)
            content = choice.message.content or ""
            msg = {"message": content}
            if hasattr(choice.message, "tool_calls") and choice.message.tool_calls:
                msg["tool_calls"] = [call.to_dict() for call in choice.message.tool_calls]
            outputs.append(msg)
    ...
    def _history_to_messages(self, history: History) -> list[dict[str, str]]:
        ...
        for item in history:
            ...
            if "tool_call_ids" in item and role == "tool":
                message["tool_call_id"] = item["tool_call_ids"][0]
            if "cache_control" in item:
                message["cache_control"] = item["cache_control"]
            messages.append(message)
        return messages
